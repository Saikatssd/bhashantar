...
const initializeDefaults = async () => {
  const defaultPermissions = [
    { links: ["/register"], view: true, add: true, edit: true, delete: true, upload: true, download: true },
  ];

  try {
    const existingRoles = await Role.find({ role_name: { $in: ['superAdmin', 'admin', 'user'] } }).lean();
